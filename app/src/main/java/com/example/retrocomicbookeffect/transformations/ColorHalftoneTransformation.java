package com.example.retrocomicbookeffect.transformations;

import android.graphics.Rect;

public class ColorHalftoneTransformation extends WholeImageTransformation {
	...
	public ColorHalftoneTransformation(float dotRadius) {
		super();
	...
	protected int[] filterPixels(int width, int height, int[] inPixels, Rect transformedSpace) {
        ...
        for ( int y = 0; y < height; y++ ) {
            ...
            for(int p = 0; p < width; p++) {
				resPixels[(y*width)+p] = outPixels[p];//inPixels[(y*width)+p];
			}
        }
	...
	@Override
	public String key() {
		return ColorHalftoneTransformation.class.getCanonicalName();
	}
	
	public String toString() {
        return "Colors/Halftone...";
    }
